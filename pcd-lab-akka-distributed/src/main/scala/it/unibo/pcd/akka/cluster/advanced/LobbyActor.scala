...
object LobbyActor:
  val lobbyKey: ServiceKey[LobbyMessage] = ServiceKey("lobby")

  def apply(): Behavior[LobbyMessage] =
    Behaviors.setup(context => LobbyActor(context))

  private case class LobbyActor(override val context: ActorContext[LobbyMessage]) extends AbstractBehavior[LobbyMessage](context):
    ...
    override def onMessage(message: LobbyMessage): Behavior[LobbyMessage] = message match
